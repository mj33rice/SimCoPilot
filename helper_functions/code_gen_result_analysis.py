...
def overwrite_first_word_in_dict_keys(d, label_mapping):
    ...
    return new_dict
class CodeAnalysis:
    def __init__(self, gen_code_pass_col='gen_code_pass_ratio', reason_analysis_col='reason_freq_analysis', horizon_analysis_col='horizon_freq_analysis', labels_reasons=None, labels_horizons=None, weighted=False, use_max_range=True):
    ...
    def analyze_csv(self, file_path, label_mapping=None, report_err_bar=True, debug_print=False):
        df = pd.read_csv(file_path)
        model_name, gen_mode, gen_code_task, gen_code_time = self.extract_info_from_filename(file_path)
        if debug_print:
            print(model_name, gen_mode, gen_code_task, gen_code_time)
