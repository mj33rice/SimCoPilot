...
def process_and_evaluate_code(src_df, args, 
    ...
    for index, row in src_df.iterrows():
        ...
        if start_line != 295 and end_line != 298:
                continue
...
def main(args, src_csv_folder, output_file_path, gen_model, code_gen_mode='no_afterlines', short_range=5, medium_range=20, 
    ...
    for src_csv_file in src_csv_files_list:
        ...
        import pdb; pdb.set_trace()
        model_name, gen_mode, gen_code_task, gen_code_time = extract_info_from_filename(src_csv_file_path)
        # print(model_name, gen_mode, gen_code_task, gen_code_time)  # Debugging line
        print('%'* 30)
        print(f"Model Name: {model_name}\nGeneration Mode: {gen_mode}\nGeneration Code Task: {gen_code_task}\nGeneration Code Time: {gen_code_time}")
        print('%'* 30)
        import pdb; pdb.set_trace()
        ...
        for index, row in src_df.iterrows():
            ...
            # if start_line != 295 and end_line != 298:
            #     continue
            if start_line != 216 and end_line != 216:
                continue
