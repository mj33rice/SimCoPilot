...
def update_checkpoints_labels(args, src_df, src_csv_file_path, print_process=True):
    # Initialize the columns with default values
    src_df['reason_categories_output'] = None
    src_df['reason_freq_analysis'] = None
    args.read_from_results = True
    args.save_dependency_results = False
    ...
    for index, row in src_df.iterrows():
        ...
        # Load the necessary columns for get_checkpoints_labels()
        before = ast.literal_eval(row['before'])
        between = ast.literal_eval(row['between'])
        after = ast.literal_eval(row['after'])
        start_line = row['start_line']
        ...
        #TODO: Parsing error when concat them together
        # Concatenate before, between, and after to form the source code
        source_code = "\n".join(before + between + after)
        try:
            if program_type == 'Java':
                analyzer = JavaDependencyAnalyzer(source_code, start_line, end_line, args.short_range, args.medium_range, update_def_line=args.update_def_line)
            else:
                analyzer = DependencyAnalyzer(source_code, start_line, end_line, args.short_range, args.medium_range, update_def_line=args.update_def_line)
        except:
            print(source_code)
            print('src_csv_file_path', src_csv_file_path)
            # import pdb;pdb.set_trace()
        if print_process:
            display_gen_code(before, between, after, '', start_line, end_line)

        # try:
        # Call get_checkpoints_labels() function
        reason_categories_output, horizon_categories_output,\
        reason_categories_printout, horizon_categories_printout,\
        horizon_freq_analysis, reason_freq_analysis = get_checkpoints_labels(args, program_type, source_code, start_line, end_line, args.short_range, args.medium_range)
        # except Exception as e:
        #     print(e)
        #     import pdb;pdb.set_trace()
        # Update the 'reason_categories_output' and 'reason_freq_analysis' columns
        src_df.at[index, 'reason_categories_output'] = reason_categories_output
        src_df.at[index, 'reason_freq_analysis'] = reason_freq_analysis
...
def main(args, src_csv_folder, output_file_path, gen_model, code_gen_mode='no_afterlines', short_range=5, medium_range=20, 
    ...
    for src_csv_file in src_csv_files_list:
        ...
        if get_task_labels:
            src_df = update_checkpoints_labels(args, src_df, src_csv_file_path)
            eval_post_process_code = False
        else:
        ...
        if get_task_labels:
            new_folder_name = 'Update_labels'
        else:
            new_folder_name = 'Updated_post_process'
        res_csv_file_path = os.path.join(src_csv_folder, rename_file(src_csv_file, new_folder_name))
...
if __name__ == "__main__":
    # try:
        ...
        parser.add_argument('--read_dependency_results', action='store_true', default=True, help='Read dependency results from a CSV file.')
